...
- TD-029 | Uso de raw query insegura no acesso a dados | Dimensão 3 — Segurança | VDI 3.95 | apps/worker/src/agents/runtime.tools.ts:92
- TD-030 | Chamada externa sem timeout ou abort path explícito | Dimensão 3 — Segurança | VDI 3.6 | apps/web/lib/sales-os/engine.ts:107
- TD-031 | Configuração sensível dispersa em módulo crítico | Dimensão 3 — Segurança | VDI 3.45 | packages/agents-core/src/tools/__mocks__/connectors.mock.ts:5
- TD-032 | Configuração sensível dispersa em módulo crítico | Dimensão 3 — Segurança | VDI 3.45 | packages/auth/index.ts:5
- TD-054 | Consulta findMany sem paginação explícita | Dimensão 5 — Performance e Escalabilidade | VDI 3.3 | apps/api/src/modules/workflows/service.ts:540

### Análise Pendente

- TD-012 | [DADOS INSUFICIENTES — REQUER: docs/architecture/c4-context.md] Diagrama C4 versionado do core | requer: docs/architecture/c4-context.md
- TD-013 | [DADOS INSUFICIENTES — REQUER: docs/architecture/dependency-boundaries.md] Matriz explícita de dependências permitidas (complementar 2) | requer: docs/architecture/dependency-boundaries.md
- TD-014 | [DADOS INSUFICIENTES — REQUER: docs/architecture/c4-context.md] Diagrama C4 versionado do core (complementar 3) | requer: docs/architecture/c4-context.md
...
- TD-050 | [DADOS INSUFICIENTES — REQUER: cobertura por módulo] Cobertura quantitativa por camada | requer: cobertura por módulo
- TD-051 | [DADOS INSUFICIENTES — REQUER: SLO dashboards vivos] SLO/SLA com erro budget operacional (complementar 2) | requer: SLO dashboards vivos
- TD-052 | [DADOS INSUFICIENTES — REQUER: cobertura por módulo] Cobertura quantitativa por camada (complementar 3) | requer: cobertura por módulo
- TD-053 | [DADOS INSUFICIENTES — REQUER: SLO dashboards vivos] SLO/SLA com erro budget operacional (complementar 4) | requer: SLO dashboards vivos
...
- TD-001 | Complexidade acima do limiar em DashboardHomePage
  Localização: apps/web/app/(dashboard)/dashboard/page.tsx:73
  Problema: Identificado em apps/web/app/(dashboard)/dashboard/page.tsx:73 uma função com complexidade ciclomática 34, acima do limiar operacional recomendado (>10).
...
- TD-002 | Complexidade acima do limiar em GlobalSearch
  Localização: apps/web/components/layout/GlobalSearch.tsx:53
  Problema: Identificado em apps/web/components/layout/GlobalSearch.tsx:53 uma função com complexidade ciclomática 21, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-003 | Complexidade acima do limiar em main
  Localização: packages/database/scripts/check-migration-governance.ts:26
  Problema: Identificado em packages/database/scripts/check-migration-governance.ts:26 uma função com complexidade ciclomática 22, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-004 | Complexidade acima do limiar em Navbar
  Localização: apps/web/components/layout/Navbar.tsx:82
  Problema: Identificado em apps/web/components/layout/Navbar.tsx:82 uma função com complexidade ciclomática 22, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-005 | Complexidade acima do limiar em SalesOsShell
  Localização: apps/web/components/sales-os/SalesOsShell.tsx:249
  Problema: Identificado em apps/web/components/sales-os/SalesOsShell.tsx:249 uma função com complexidade ciclomática 73, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-006 | Complexidade acima do limiar em usePatientDetailModel
  Localização: apps/web/app/(dashboard)/patients/[id]/page.tsx:130
  Problema: Identificado em apps/web/app/(dashboard)/patients/[id]/page.tsx:130 uma função com complexidade ciclomática 44, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-007 | Superfície legacy ainda versionada ao lado do core canônico
...
- TD-008 | Arquivo grande demais para o boundary atual (1287 linhas)
  Localização: packages/database/prisma/schema.prisma:1
  Problema: Identificado em packages/database/prisma/schema.prisma um arquivo com 1287 linhas dentro do core, sinal típico de boundary inchado ou múltiplas responsabilidades.
...
- TD-009 | Arquivo grande demais para o boundary atual (2345 linhas)
  Localização: apps/api/src/modules/clinical/service.ts:1
  Problema: Identificado em apps/api/src/modules/clinical/service.ts um arquivo com 2345 linhas dentro do core, sinal típico de boundary inchado ou múltiplas responsabilidades.
  Impacto: Arquivos extensos concentram conhecimento, elevam custo de merge e pioram isolamento de testes.
  Solução recomendada: Separar orchestration, adapters e regras de negócio em módulos menores alinhados ao boundary funcional.
  VDI: 2.75 (MÉDIO)
  Esforço: 2-5 dias

- TD-010 | Arquivo grande demais para o boundary atual (6945 linhas)
  Localização: apps/web/lib/sales-os/constants.ts:1
  Problema: Identificado em apps/web/lib/sales-os/constants.ts um arquivo com 6945 linhas dentro do core, sinal típico de boundary inchado ou múltiplas responsabilidades.
  Impacto: Arquivos extensos concentram conhecimento, elevam custo de merge e pioram isolamento de testes.
  Solução recomendada: Separar orchestration, adapters e regras de negócio em módulos menores alinhados ao boundary funcional.
  VDI: 2.75 (MÉDIO)
  Esforço: 2-5 dias

- TD-011 | Arquivo grande demais para o boundary atual (976 linhas)
  Localização: packages/database/prisma/migration-registry.json:1
  Problema: Identificado em packages/database/prisma/migration-registry.json um arquivo com 976 linhas dentro do core, sinal típico de boundary inchado ou múltiplas responsabilidades.
  Impacto: Arquivos extensos concentram conhecimento, elevam custo de merge e pioram isolamento de testes.
  Solução recomendada: Separar orchestration, adapters e regras de negócio em módulos menores alinhados ao boundary funcional.
  VDI: 2.75 (MÉDIO)
  Esforço: 2-5 dias

- TD-012 | [DADOS INSUFICIENTES — REQUER: docs/architecture/c4-context.md] Diagrama C4 versionado do core
  Localização: docs/service-catalog.md:1
...
- TD-013 | [DADOS INSUFICIENTES — REQUER: docs/architecture/dependency-boundaries.md] Matriz explícita de dependências permitidas (complementar 2)
  Localização: docs/service-catalog.md:1
  Problema: Não há uma matriz versionada que formalize as dependências permitidas entre apps, packages e superfícies legadas/quarentenadas.
  Impacto: Sem regra publicada, acoplamentos regressivos podem voltar ao core sem gatilho documental claro.
  Solução recomendada: Publicar uma matriz de dependências permitidas e bloqueadas entre o core, satélites e legado.
  VDI: 2.35 (MÉDIO)
  Esforço: 0.5-1 dia para materializar a evidência; maior se a capacidade não existir.

- TD-014 | [DADOS INSUFICIENTES — REQUER: docs/architecture/c4-context.md] Diagrama C4 versionado do core (complementar 3)
  Localização: docs/service-catalog.md:1
  Problema: O repositório lista a taxonomia canônica do core, mas não traz um diagrama C4 versionado do contexto atual para validar fronteiras, responsabilidades e integrações síncronas/assíncronas.
  Impacto: Sem essa evidência, revisões arquiteturais ficam dependentes de leitura de código e aumentam o custo de onboarding e de refactors transversais.
  Solução recomendada: Versionar um diagrama C4 Context + Container do core canônico e referenciá-lo a partir do service catalog.
...
- TD-015 | Acesso direto a process.env fora da camada de configuração
  Localização: apps/web/lib/sales-os/engine.ts:50
  Problema: Identificado em apps/web/lib/sales-os/engine.ts:50 acesso direto a process.env em runtime fora de um boundary dedicado de config.
...
- TD-016 | Acesso direto a process.env fora da camada de configuração
  Localização: packages/database/src/client.ts:51
  Problema: Identificado em packages/database/src/client.ts:51 acesso direto a process.env em runtime fora de um boundary dedicado de config.
  Impacto: Isso dispersa contrato de configuração, dificulta validação centralizada e fragiliza testes/observabilidade.
  Solução recomendada: Encapsular leituras em surface única de configuração com validação e defaults explícitos.
  VDI: 2.95 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-017 | Acesso direto a process.env fora da camada de configuração
  Localização: apps/worker/src/agents/runtime.db-integration.harness.ts:53
  Problema: Identificado em apps/worker/src/agents/runtime.db-integration.harness.ts:53 acesso direto a process.env em runtime fora de um boundary dedicado de config.
  Impacto: Isso dispersa contrato de configuração, dificulta validação centralizada e fragiliza testes/observabilidade.
  Solução recomendada: Encapsular leituras em surface única de configuração com validação e defaults explícitos.
  VDI: 2.95 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-018 | Acesso direto a process.env fora da camada de configuração
  Localização: packages/logger/src/index.ts:63
  Problema: Identificado em packages/logger/src/index.ts:63 acesso direto a process.env em runtime fora de um boundary dedicado de config.
  Impacto: Isso dispersa contrato de configuração, dificulta validação centralizada e fragiliza testes/observabilidade.
  Solução recomendada: Encapsular leituras em surface única de configuração com validação e defaults explícitos.
  VDI: 2.95 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-019 | Acesso direto a process.env fora da camada de configuração
  Localização: packages/database/scripts/compare-migration-state.ts:17
  Problema: Identificado em packages/database/scripts/compare-migration-state.ts:17 acesso direto a process.env em runtime fora de um boundary dedicado de config.
  Impacto: Isso dispersa contrato de configuração, dificulta validação centralizada e fragiliza testes/observabilidade.
  Solução recomendada: Encapsular leituras em surface única de configuração com validação e defaults explícitos.
  VDI: 2.95 (MÉDIO)
  Esforço: 0.5-2 dias

- TD-020 | Uso recorrente de any em agent.ts
  Localização: packages/agents/executivos/boardprep-ai/agent.ts:788
  Problema: Identificado em packages/agents/executivos/boardprep-ai/agent.ts:788 um cluster de 1 ocorrência(s) de any em código de runtime.
...
- TD-021 | Uso recorrente de any em encryption.ts
  Localização: apps/api/src/lib/encryption.ts:21
  Problema: Identificado em apps/api/src/lib/encryption.ts:21 um cluster de 1 ocorrência(s) de any em código de runtime.
  Impacto: Esse padrão reduz garantias de tipo justamente nos pontos onde o core deveria ser mais explícito e previsível.
  Solução recomendada: Substituir any por unions, schemas zod ou tipos derivados das entidades reais trafegadas pelo módulo.
  VDI: 2.85 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-022 | Uso recorrente de any em index.ts
  Localização: packages/queue/src/index.ts:29
  Problema: Identificado em packages/queue/src/index.ts:29 um cluster de 2 ocorrência(s) de any em código de runtime.
  Impacto: Esse padrão reduz garantias de tipo justamente nos pontos onde o core deveria ser mais explícito e previsível.
  Solução recomendada: Substituir any por unions, schemas zod ou tipos derivados das entidades reais trafegadas pelo módulo.
  VDI: 2.85 (MÉDIO)
  Esforço: 0.5-2 dias

- TD-023 | Uso recorrente de any em main.js
  Localização: packages/agents-core/docs/assets/main.js:5
  Problema: Identificado em packages/agents-core/docs/assets/main.js:5 um cluster de 1 ocorrência(s) de any em código de runtime.
  Impacto: Esse padrão reduz garantias de tipo justamente nos pontos onde o core deveria ser mais explícito e previsível.
  Solução recomendada: Substituir any por unions, schemas zod ou tipos derivados das entidades reais trafegadas pelo módulo.
  VDI: 2.85 (MÉDIO)
  Esforço: 0.5-2 dias

- TD-024 | Uso recorrente de any em pack-installer.service.ts
  Localização: apps/api/src/modules/packs/pack-installer.service.ts:306
  Problema: Identificado em apps/api/src/modules/packs/pack-installer.service.ts:306 um cluster de 1 ocorrência(s) de any em código de runtime.
  Impacto: Esse padrão reduz garantias de tipo justamente nos pontos onde o core deveria ser mais explícito e previsível.
  Solução recomendada: Substituir any por unions, schemas zod ou tipos derivados das entidades reais trafegadas pelo módulo.
  VDI: 2.85 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-025 | Uso recorrente de any em signatures.ts
  Localização: packages/integrations/src/clients/signatures.ts:76
  Problema: Identificado em packages/integrations/src/clients/signatures.ts:76 um cluster de 2 ocorrência(s) de any em código de runtime.
  Impacto: Esse padrão reduz garantias de tipo justamente nos pontos onde o core deveria ser mais explícito e previsível.
  Solução recomendada: Substituir any por unions, schemas zod ou tipos derivados das entidades reais trafegadas pelo módulo.
  VDI: 2.85 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-026 | Logging ad-hoc com console.* em runtime
  Localização: packages/agents/executivos/boardprep-ai/agent.ts:433
  Problema: Identificado em packages/agents/executivos/boardprep-ai/agent.ts:433 uso de console.* em uma superfície de runtime do core.
...
- TD-027 | Logging ad-hoc com console.* em runtime
  Localização: packages/agents/executivos/brand-guardian/agent.ts:517
  Problema: Identificado em packages/agents/executivos/brand-guardian/agent.ts:517 uso de console.* em uma superfície de runtime do core.
  Impacto: Esse padrão dificulta padronização de logs, correlação por trace id e aplicação consistente de redaction.
  Solução recomendada: Migrar o módulo para o logger estruturado do workspace e eliminar saídas diretas via console.
  VDI: 2.6 (MÉDIO)
  Esforço: 0.5-1 dia
...
- TD-028 | Logging ad-hoc com console.* em runtime
  Localização: packages/agents/executivos/budget-fluid/agent.ts:515
  Problema: Identificado em packages/agents/executivos/budget-fluid/agent.ts:515 uso de console.* em uma superfície de runtime do core.
  Impacto: Esse padrão dificulta padronização de logs, correlação por trace id e aplicação consistente de redaction.
  Solução recomendada: Migrar o módulo para o logger estruturado do workspace e eliminar saídas diretas via console.
  VDI: 2.6 (MÉDIO)
  Esforço: 0.5-1 dia
...
- TD-029 | Uso de raw query insegura no acesso a dados
  Localização: apps/worker/src/agents/runtime.tools.ts:92
  Problema: Identificado em apps/worker/src/agents/runtime.tools.ts:92 uso de prisma raw unsafe, o que amplia superfície para injection ou bypass de abstrações seguras.
  Impacto: Esse padrão aumenta risco de falha crítica de segurança e dificulta validar tenancy e saneamento em todo call path.
  Solução recomendada: Substituir por query parametrizada segura ou reintroduzir a operação em repository tipado com validação explícita.
  VDI: 3.95 (ALTO)
  Esforço: 1-3 dias

- TD-030 | Chamada externa sem timeout ou abort path explícito
  Localização: apps/web/lib/sales-os/engine.ts:107
  Problema: Indício em apps/web/lib/sales-os/engine.ts:107 de acesso externo sem timeout explícito no arquivo.
...
- TD-031 | Configuração sensível dispersa em módulo crítico
  Localização: packages/agents-core/src/tools/__mocks__/connectors.mock.ts:5
  Problema: Identificado em módulo crítico (packages/agents-core/src/tools/__mocks__/connectors.mock.ts) acesso direto a process.env, reduzindo auditabilidade de segredos e políticas de fallback.
  ...
  Solução recomendada: Centralizar segredos/flags operacionais em config tipada e remover leitura direta no módulo de domínio.
  VDI: 3.45 (ALTO)
  Esforço: 0.5-2 dias

- TD-032 | Configuração sensível dispersa em módulo crítico
  Localização: packages/auth/index.ts:5
  Problema: Identificado em módulo crítico (packages/auth/index.ts) acesso direto a process.env, reduzindo auditabilidade de segredos e políticas de fallback.
  Impacto: Módulos críticos com leitura dispersa de env facilitam drift entre ambientes e tornam segredos mais difíceis de governar.
  Solução recomendada: Centralizar segredos/flags operacionais em config tipada e remover leitura direta no módulo de domínio.
  VDI: 3.45 (ALTO)
  Esforço: 0.5-2 dias

- TD-033 | Semgrep WARNING em prime-collect.mjs
  Localização: scripts/audit/prime-collect.mjs:267
  Problema: Semgrep sinalizou em scripts/audit/prime-collect.mjs:267 o padrão "RegExp() called with a `{ id, regex, filter, summary }` function argument, this might allow an attacker to cause a Regular Expression Denial-of-Service (ReDoS) within your application as RegExP blocks the main thread".
...
- TD-034 | Semgrep INFO em service.yaml
  Localização: infra/cloudrun/service.yaml:10
  Problema: Semgrep sinalizou em infra/cloudrun/service.yaml:10 o padrão "When running containers in Kubernetes, it's important to ensure that they  are properly secured to prevent privilege escalation attacks".
  Impacto: Esse tipo de finding amplia a superfície de exploração e indica controles de segurança ainda incompletos no HEAD atual.
  Solução recomendada: Aplicar a correção sugerida pelo Semgrep e validar o fluxo afetado. Fix sugerido: spec:
      securityContext:
        runAsNonRoot: true #
  VDI: 2.8 (MÉDIO)
  Esforço: 0.5-2 dias

- TD-035 | Semgrep INFO em shared-prime.mjs
  Localização: scripts/audit/shared-prime.mjs:385
  Problema: Semgrep sinalizou em scripts/audit/shared-prime.mjs:385 o padrão "Detected a call to `replaceAll()` in an attempt to HTML escape the string `String(value)`".
  Impacto: Esse tipo de finding amplia a superfície de exploração e indica controles de segurança ainda incompletos no HEAD atual.
  Solução recomendada: Endereçar o padrão sinalizado pelo Semgrep e adicionar cobertura de regressão para o fluxo afetado.
  VDI: 2.8 (MÉDIO)
  Esforço: 0.5-2 dias

- TD-036 | Semgrep INFO em shared-prime.mjs
  Localização: scripts/audit/shared-prime.mjs:385
  Problema: Semgrep sinalizou em scripts/audit/shared-prime.mjs:385 o padrão "Detected a call to `replaceAll()` in an attempt to HTML escape the string `String(value)
    .replaceAll("&", "&amp;")`".
  Impacto: Esse tipo de finding amplia a superfície de exploração e indica controles de segurança ainda incompletos no HEAD atual.
...
- TD-037 | Semgrep INFO em shared-prime.mjs
  Localização: scripts/audit/shared-prime.mjs:385
  Problema: Semgrep sinalizou em scripts/audit/shared-prime.mjs:385 o padrão "Detected a call to `replaceAll()` in an attempt to HTML escape the string `String(value)
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")`".
...
- TD-042 | Observabilidade inconsistente por uso de console em runtime
  Localização: packages/agents/executivos/boardprep-ai/agent.ts:433
  Problema: Identificado em packages/agents/executivos/boardprep-ai/agent.ts:433 logging ad-hoc fora da cadeia estruturada de observabilidade.
...
- TD-043 | Observabilidade inconsistente por uso de console em runtime
  Localização: packages/agents/executivos/brand-guardian/agent.ts:517
  Problema: Identificado em packages/agents/executivos/brand-guardian/agent.ts:517 logging ad-hoc fora da cadeia estruturada de observabilidade.
  Impacto: Sinais operacionais ficam parciais e dificultam correlação entre logs, métricas e traces durante incidentes.
  Solução recomendada: Alinhar o módulo ao logger estruturado e adicionar contexto de tenant/trace nas saídas relevantes.
  VDI: 2.95 (MÉDIO)
  Esforço: 0.5-1 dia
...
- TD-044 | Cobertura estrutural baixa em apps/web
  ...
  Problema: O proxy de cobertura identifica 137 arquivos de runtime para apps/web, mas apenas 23 arquivos de teste diretos e 4 gaps principais.
...
- TD-045 | Cobertura estrutural baixa em apps/worker
  ...
  Problema: O proxy de cobertura identifica 79 arquivos de runtime para apps/worker, mas apenas 32 arquivos de teste diretos e 1 gaps principais.
  Impacto: Com poucos testes diretos para módulos extensos, regressões operacionais e de observabilidade tendem a aparecer tarde no ciclo.
  Solução recomendada: Priorizar suites unit/integration nos primeiros arquivos do gap e anexar cobertura quantitativa real ao lane soberano.
  VDI: 2.85 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-046 | Módulo volumoso sem teste relacionado direto
  Localização: apps/web/app/(dashboard)/patients/[id]/page.sections.tsx:1
  Problema: Identificado em apps/web/app/(dashboard)/patients/[id]/page.sections.tsx um módulo relevante sem teste relacionado por heurística de nome/stem no workspace atual.
...
- TD-049 | Módulo volumoso sem teste relacionado direto
  Localização: apps/web/components/layout/GlobalSearch.tsx:1
  Problema: Identificado em apps/web/components/layout/GlobalSearch.tsx um módulo relevante sem teste relacionado por heurística de nome/stem no workspace atual.
  Impacto: A chance de regressão silenciosa cresce quando módulos grandes ou críticos não têm suite vinculada de forma óbvia.
  Solução recomendada: Criar pelo menos um teste unitário ou integration diretamente associado ao boundary do arquivo.
  VDI: 2.65 (MÉDIO)
  Esforço: 0.5-2 dias

- TD-050 | [DADOS INSUFICIENTES — REQUER: cobertura por módulo] Cobertura quantitativa por camada
...
- TD-051 | [DADOS INSUFICIENTES — REQUER: SLO dashboards vivos] SLO/SLA com erro budget operacional (complementar 2)
...
- TD-052 | [DADOS INSUFICIENTES — REQUER: cobertura por módulo] Cobertura quantitativa por camada (complementar 3)
  Localização: scripts/testing/generate-traceability-report.mjs:1
  Problema: Há muitas suites de teste no monorepo, mas a auditoria não encontra uma matriz de cobertura unit/integration/e2e por módulo do core no formato atual.
  Impacto: Sem a decomposição por módulo, o risco de falso conforto sobre cobertura global aumenta.
  Solução recomendada: Gerar um relatório de cobertura por módulo e anexar ao pipeline soberano.
...
- TD-053 | [DADOS INSUFICIENTES — REQUER: SLO dashboards vivos] SLO/SLA com erro budget operacional (complementar 4)
  Localização: infra/monitoring/alert.rules.yml:1
  Problema: Há artefatos e regras de monitoramento versionados, mas não há evidência viva de dashboards/SLOs com budget consumido por janela.
  Impacto: A operação fica com sinais técnicos, mas sem indicador explícito de confiabilidade do serviço para decisão executiva.
  Solução recomendada: Versionar a definição de SLO por fluxo crítico e anexar screenshot/export JSON dos dashboards vivos.
...
- TD-054 | Consulta findMany sem paginação explícita
  Localização: apps/api/src/modules/workflows/service.ts:540
  Problema: Indício em apps/api/src/modules/workflows/service.ts:540 de uso de findMany sem take/skip/cursor nas linhas adjacentes.
...
- TD-055 | Consulta findMany sem paginação explícita
  Localização: apps/api/src/modules/workflows/service.ts:580
  Problema: Indício em apps/api/src/modules/workflows/service.ts:580 de uso de findMany sem take/skip/cursor nas linhas adjacentes.
  Impacto: Consultas amplas degradam latência, aumentam custo de banco e pioram risco de DoS por leitura excessiva.
  Solução recomendada: Adicionar paginação explícita, limites defensivos e métricas por rota/serviço que consome a query.
  VDI: 3.3 (ALTO)
  Esforço: 0.5-2 dias
...
- TD-056 | Consulta findMany sem paginação explícita
  Localização: apps/api/src/modules/workflows/service.ts:758
  Problema: Indício em apps/api/src/modules/workflows/service.ts:758 de uso de findMany sem take/skip/cursor nas linhas adjacentes.
  Impacto: Consultas amplas degradam latência, aumentam custo de banco e pioram risco de DoS por leitura excessiva.
  Solução recomendada: Adicionar paginação explícita, limites defensivos e métricas por rota/serviço que consome a query.
  VDI: 3.3 (ALTO)
  Esforço: 0.5-2 dias
...
- TD-057 | Consulta findMany sem paginação explícita
  Localização: apps/api/src/modules/workflows/service.ts:806
  Problema: Indício em apps/api/src/modules/workflows/service.ts:806 de uso de findMany sem take/skip/cursor nas linhas adjacentes.
  Impacto: Consultas amplas degradam latência, aumentam custo de banco e pioram risco de DoS por leitura excessiva.
  Solução recomendada: Adicionar paginação explícita, limites defensivos e métricas por rota/serviço que consome a query.
  VDI: 3.3 (ALTO)
  Esforço: 0.5-2 dias
...
- TD-058 | Await serial em loop de runtime
  Localização: apps/api/src/modules/privacy/consent.service.ts:191
  Problema: Indício em apps/api/src/modules/privacy/consent.service.ts:191 de await dentro de loop sequencial.
...
- TD-059 | Await serial em loop de runtime
  Localização: apps/api/src/modules/privacy/retention.service.ts:117
  Problema: Indício em apps/api/src/modules/privacy/retention.service.ts:117 de await dentro de loop sequencial.
  Impacto: Esse padrão alonga tempo de resposta e throughput do worker/API, principalmente sob carga ou fan-out externo.
  Solução recomendada: Avaliar paralelização controlada, batching ou filas dedicadas com limites explícitos.
  VDI: 3.05 (ALTO)
  Esforço: 1-3 dias
...
- TD-060 | Await serial em loop de runtime
  Localização: apps/api/src/modules/privacy/retention.service.ts:166
  Problema: Indício em apps/api/src/modules/privacy/retention.service.ts:166 de await dentro de loop sequencial.
  Impacto: Esse padrão alonga tempo de resposta e throughput do worker/API, principalmente sob carga ou fan-out externo.
  Solução recomendada: Avaliar paralelização controlada, batching ou filas dedicadas com limites explícitos.
  VDI: 3.05 (ALTO)
  Esforço: 1-3 dias
...
- TD-061 | Await serial em loop de runtime
  Localização: apps/api/src/modules/privacy/retention.service.ts:231
  Problema: Indício em apps/api/src/modules/privacy/retention.service.ts:231 de await dentro de loop sequencial.
  Impacto: Esse padrão alonga tempo de resposta e throughput do worker/API, principalmente sob carga ou fan-out externo.
  Solução recomendada: Avaliar paralelização controlada, batching ou filas dedicadas com limites explícitos.
  VDI: 3.05 (ALTO)
  Esforço: 1-3 dias
...
- TD-062 | Integração externa sem deadline operacional explícito
  Localização: apps/web/lib/sales-os/engine.ts:107
  Problema: Acesso externo em apps/web/lib/sales-os/engine.ts:107 não expõe timeout/abort guard no arquivo atual.
...
- TD-063 | Bundle web fresco acima de 2 MiB (2443.96 KiB)
  Localização: artifacts/performance/web-bundle-head.json:1
  Problema: A baseline fresca do bundle registrou 2443.96 KiB distribuídos em 65 arquivos, com chunk líder de 480605 bytes.
...
- TD-064 | Página/estado web com complexidade alta (34)
  Localização: apps/web/app/(dashboard)/dashboard/page.tsx:73
  Problema: Identificado em apps/web/app/(dashboard)/dashboard/page.tsx:73 um componente/página com complexidade 34.
...
- TD-065 | Página/estado web com complexidade alta (44)
  Localização: apps/web/app/(dashboard)/patients/[id]/page.tsx:130
  Problema: Identificado em apps/web/app/(dashboard)/patients/[id]/page.tsx:130 um componente/página com complexidade 44.
  Impacto: Componentes muito complexos prejudicam rendering predictability, manutenção de loading/error states e otimização futura de bundle.
  Solução recomendada: Separar data-loading, rendering e actions do componente em camadas menores e mais previsíveis.
  VDI: 2.4 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-067 | Dockerfile sem evidência de multi-stage build
  Localização: artifacts/audit/files_analysis/apps/api/Dockerfile.md:1
  Problema: Identificado em artifacts/audit/files_analysis/apps/api/Dockerfile.md apenas 0 instrução(ões) FROM.
...
- TD-068 | Dockerfile sem evidência de multi-stage build
  Localização: artifacts/audit/files_analysis/apps/web/Dockerfile.md:1
  Problema: Identificado em artifacts/audit/files_analysis/apps/web/Dockerfile.md apenas 0 instrução(ões) FROM.
  Impacto: Imagens single-stage tendem a carregar toolchain desnecessária, elevar superfície de ataque e aumentar tempo de pull/deploy.
  Solução recomendada: Converter o Dockerfile para multi-stage com base enxuta e assets finais mínimos.
  VDI: 2.8 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-069 | Dockerfile sem evidência de multi-stage build
  Localização: artifacts/audit/files_analysis/apps/worker/Dockerfile.md:1
  Problema: Identificado em artifacts/audit/files_analysis/apps/worker/Dockerfile.md apenas 0 instrução(ões) FROM.
  Impacto: Imagens single-stage tendem a carregar toolchain desnecessária, elevar superfície de ataque e aumentar tempo de pull/deploy.
  Solução recomendada: Converter o Dockerfile para multi-stage com base enxuta e assets finais mínimos.
  VDI: 2.8 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-081 | Strings de interface hardcoded em superfície web
  Localização: apps/web/app/(dashboard)/agents/page.tsx:8
  Problema: Identificado em apps/web/app/(dashboard)/agents/page.tsx:8 um cluster de 46 linhas com texto hardcoded na UI.
...
- TD-082 | Strings de interface hardcoded em superfície web
  Localização: apps/web/app/(dashboard)/analytics/page.tsx:9
  Problema: Identificado em apps/web/app/(dashboard)/analytics/page.tsx:9 um cluster de 33 linhas com texto hardcoded na UI.
  Impacto: Esse padrão reduz prontidão para i18n, dificulta consistência editorial e aumenta custo de manutenção de UX.
  Solução recomendada: Extrair strings para camada de mensagens/localização com organização por rota e contexto de uso.
  VDI: 2.55 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-083 | Strings de interface hardcoded em superfície web
  Localização: apps/web/app/(dashboard)/billing/budgets/page.tsx:11
  Problema: Identificado em apps/web/app/(dashboard)/billing/budgets/page.tsx:11 um cluster de 29 linhas com texto hardcoded na UI.
  Impacto: Esse padrão reduz prontidão para i18n, dificulta consistência editorial e aumenta custo de manutenção de UX.
  Solução recomendada: Extrair strings para camada de mensagens/localização com organização por rota e contexto de uso.
  VDI: 2.55 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-084 | Strings de interface hardcoded em superfície web
  Localização: apps/web/app/(dashboard)/conversations/page.tsx:3
  Problema: Identificado em apps/web/app/(dashboard)/conversations/page.tsx:3 um cluster de 84 linhas com texto hardcoded na UI.
  Impacto: Esse padrão reduz prontidão para i18n, dificulta consistência editorial e aumenta custo de manutenção de UX.
  Solução recomendada: Extrair strings para camada de mensagens/localização com organização por rota e contexto de uso.
  VDI: 2.55 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-085 | Strings de interface hardcoded em superfície web
  Localização: apps/web/app/(dashboard)/dashboard/page.tsx:8
  Problema: Identificado em apps/web/app/(dashboard)/dashboard/page.tsx:8 um cluster de 39 linhas com texto hardcoded na UI.
  Impacto: Esse padrão reduz prontidão para i18n, dificulta consistência editorial e aumenta custo de manutenção de UX.
  Solução recomendada: Extrair strings para camada de mensagens/localização com organização por rota e contexto de uso.
  VDI: 2.55 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-086 | Strings de interface hardcoded em superfície web
  Localização: apps/web/app/(dashboard)/developers/apikeys/page.tsx:3
  Problema: Identificado em apps/web/app/(dashboard)/developers/apikeys/page.tsx:3 um cluster de 44 linhas com texto hardcoded na UI.
  Impacto: Esse padrão reduz prontidão para i18n, dificulta consistência editorial e aumenta custo de manutenção de UX.
  Solução recomendada: Extrair strings para camada de mensagens/localização com organização por rota e contexto de uso.
  VDI: 2.55 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-015 -> TD-031 (Debt dependency)
- TD-015 -> TD-032 (Debt dependency)
