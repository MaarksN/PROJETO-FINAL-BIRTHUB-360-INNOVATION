...
| `docs/roadmap.md` | Q2 BI: `BOARD_REPORT` agendado na segunda às 8h | `apps/agent-orchestrator/orchestrator/flows.py` implementa `FLOW_BOARD_REPORT_GRAPH`; `packages/queue/src/definitions.ts` usa cron `0 17 * * 5` | 🔀 DESVIO | O relatório existe, mas o agendamento encontrado aponta sexta-feira às 17h, não segunda às 8h |
...
| `BOARD_REPORT` agendado diverge do roadmap | `docs/roadmap.md` | Agenda executiva e operação analítica podem rodar fora da janela prometida | Alta | Alinhar cron real com o roadmap ou corrigir o roadmap para refletir a agenda de produção |
...
- Principais desvios identificados:
  - `BOARD_REPORT` com agendamento divergente do roadmap
