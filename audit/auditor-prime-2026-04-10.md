...
- TD-002 | Complexidade acima do limiar em DashboardHomePage | Dimensão 1 — Saúde Arquitetural | VDI 3.25 | apps/web/app/(dashboard)/dashboard/page.tsx:72
- TD-003 | Complexidade acima do limiar em GlobalSearch | Dimensão 1 — Saúde Arquitetural | VDI 3.25 | apps/web/components/layout/GlobalSearch.tsx:52
- TD-004 | Complexidade acima do limiar em main | Dimensão 1 — Saúde Arquitetural | VDI 3.25 | packages/database/scripts/check-migration-governance.ts:26
- TD-005 | Complexidade acima do limiar em Navbar | Dimensão 1 — Saúde Arquitetural | VDI 3.25 | apps/web/components/layout/Navbar.tsx:82
...
- TD-002 | Complexidade acima do limiar em DashboardHomePage
  Localização: apps/web/app/(dashboard)/dashboard/page.tsx:72
  Problema: Identificado em apps/web/app/(dashboard)/dashboard/page.tsx:72 uma função com complexidade ciclomática 34, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-003 | Complexidade acima do limiar em GlobalSearch
  Localização: apps/web/components/layout/GlobalSearch.tsx:52
  Problema: Identificado em apps/web/components/layout/GlobalSearch.tsx:52 uma função com complexidade ciclomática 21, acima do limiar operacional recomendado (>10).
  Impacto: Hotspots assim aumentam risco de regressão, dificultam revisão e ampliam o custo de mudança em fluxos centrais.
  Solução recomendada: Fatiar a função em sub-rotinas orientadas por decisão e isolar políticas/branches em helpers com testes diretos.
  VDI: 3.25 (ALTO)
  Esforço: 2-5 dias

- TD-004 | Complexidade acima do limiar em main
...
- TD-005 | Complexidade acima do limiar em Navbar
...
- TD-006 | Complexidade acima do limiar em PatientDetailPage
...
- TD-007 | Complexidade acima do limiar em PatientsPage
...
- TD-008 | Complexidade acima do limiar em SalesOsShell
...
- TD-010 | Arquivo grande demais para o boundary atual (1287 linhas)
...
- TD-011 | Arquivo grande demais para o boundary atual (2348 linhas)
  Localização: apps/api/src/modules/clinical/service.ts:1
  Problema: Identificado em apps/api/src/modules/clinical/service.ts um arquivo com 2348 linhas dentro do core, sinal típico de boundary inchado ou múltiplas responsabilidades.
  Impacto: Arquivos extensos concentram conhecimento, elevam custo de merge e pioram isolamento de testes.
  Solução recomendada: Separar orchestration, adapters e regras de negócio em módulos menores alinhados ao boundary funcional.
  VDI: 2.75 (MÉDIO)
  Esforço: 2-5 dias

- TD-012 | Arquivo grande demais para o boundary atual (6945 linhas)
...
- TD-013 | Arquivo grande demais para o boundary atual (976 linhas)
...
- TD-019 | Acesso direto a process.env fora da camada de configuração
  Localização: packages/database/scripts/provision-rls-runtime-role.ts:33
  Problema: Identificado em packages/database/scripts/provision-rls-runtime-role.ts:33 acesso direto a process.env em runtime fora de um boundary dedicado de config.
  Impacto: Isso dispersa contrato de configuração, dificulta validação centralizada e fragiliza testes/observabilidade.
  Solução recomendada: Encapsular leituras em surface única de configuração com validação e defaults explícitos.
  VDI: 2.95 (MÉDIO)
  Esforço: 0.5-2 dias
...
- TD-030 | Semgrep INFO em shared-prime.mjs
...
- TD-031 | Semgrep INFO em shared-prime.mjs
  Localização: scripts/audit/shared-prime.mjs:385
  Problema: Semgrep sinalizou em scripts/audit/shared-prime.mjs:385 o padrão "Detected a call to `replaceAll()` in an attempt to HTML escape the string `String(value)
    .replaceAll("&", "&amp;")`".
  Impacto: Esse tipo de finding amplia a superfície de exploração e indica controles de segurança ainda incompletos no HEAD atual.
...
- TD-044 | Cobertura estrutural baixa em apps/web
  ...
  Problema: O proxy de cobertura identifica 138 arquivos de runtime para apps/web, mas apenas 23 arquivos de teste diretos e 4 gaps principais.
...
- TD-054 | Await serial em loop de runtime
  Localização: apps/api/src/modules/privacy/retention.service.ts:523
  Problema: Indício em apps/api/src/modules/privacy/retention.service.ts:523 de await dentro de loop sequencial.
...
- TD-055 | Await serial em loop de runtime
  Localização: apps/api/src/modules/privacy/retention.service.ts:534
  Problema: Indício em apps/api/src/modules/privacy/retention.service.ts:534 de await dentro de loop sequencial.
  Impacto: Esse padrão alonga tempo de resposta e throughput do worker/API, principalmente sob carga ou fan-out externo.
  Solução recomendada: Avaliar paralelização controlada, batching ou filas dedicadas com limites explícitos.
  VDI: 3.05 (ALTO)
  Esforço: 1-3 dias
...
- TD-056 | Await serial em loop de runtime
  Localização: apps/api/src/modules/webhooks/eventBus.ts:44
  Problema: Indício em apps/api/src/modules/webhooks/eventBus.ts:44 de await dentro de loop sequencial.
  Impacto: Esse padrão alonga tempo de resposta e throughput do worker/API, principalmente sob carga ou fan-out externo.
  Solução recomendada: Avaliar paralelização controlada, batching ou filas dedicadas com limites explícitos.
  VDI: 3.05 (ALTO)
  Esforço: 1-3 dias
...
- TD-057 | Await serial em loop de runtime
  Localização: apps/api/src/modules/workflows/service.ts:246
  Problema: Indício em apps/api/src/modules/workflows/service.ts:246 de await dentro de loop sequencial.
  Impacto: Esse padrão alonga tempo de resposta e throughput do worker/API, principalmente sob carga ou fan-out externo.
  Solução recomendada: Avaliar paralelização controlada, batching ou filas dedicadas com limites explícitos.
  VDI: 3.05 (ALTO)
  Esforço: 1-3 dias

- TD-058 | Bundle web fresco acima de 2 MiB (2443.96 KiB)
...
- TD-060 | Página/estado web com complexidade alta (34)
  Localização: apps/web/app/(dashboard)/dashboard/page.tsx:72
  Problema: Identificado em apps/web/app/(dashboard)/dashboard/page.tsx:72 um componente/página com complexidade 34.
  Impacto: Componentes muito complexos prejudicam rendering predictability, manutenção de loading/error states e otimização futura de bundle.
  Solução recomendada: Separar data-loading, rendering e actions do componente em camadas menores e mais previsíveis.
  VDI: 2.4 (MÉDIO)
  Esforço: 1-3 dias

- TD-061 | Página/estado web com complexidade alta (73)
  Localização: apps/web/components/sales-os/SalesOsShell.tsx:249
  Problema: Identificado em apps/web/components/sales-os/SalesOsShell.tsx:249 um componente/página com complexidade 73.
  Impacto: Componentes muito complexos prejudicam rendering predictability, manutenção de loading/error states e otimização futura de bundle.
  Solução recomendada: Separar data-loading, rendering e actions do componente em camadas menores e mais previsíveis.
  VDI: 2.4 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-085 | Strings de interface hardcoded em superfície web
  ...
  Problema: Identificado em apps/web/app/(dashboard)/dashboard/page.tsx:7 um cluster de 39 linhas com texto hardcoded na UI.
  Impacto: Esse padrão reduz prontidão para i18n, dificulta consistência editorial e aumenta custo de manutenção de UX.
  Solução recomendada: Extrair strings para camada de mensagens/localização com organização por rota e contexto de uso.
  VDI: 2.55 (MÉDIO)
  Esforço: 1-3 dias
...
- TD-089 | Drill de disaster recovery não registrado no ciclo atual
  ...
  Problema: O artefato fresco de DR está em status "blocked", sem comprovação recente de exercício de recuperação.
