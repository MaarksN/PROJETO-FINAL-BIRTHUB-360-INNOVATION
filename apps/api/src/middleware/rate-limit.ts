...
async function recordBruteForceAlert(request: Request): Promise<void> {
  const tenantReference =
    typeof request.body?.tenantId === "string" ? request.body.tenantId.trim() : null;
  const email = typeof request.body?.email === "string" ? request.body.email.trim() : null;
...
export function createLoginRateLimitMiddleware(config: ApiConfig): RequestHandler {
  return rateLimit({
    ...
    keyGenerator: (request) => {
      const email =
        typeof request.body?.email === "string" && request.body.email.trim().length > 0
          ? request.body.email.trim().toLowerCase()
          : "unknown-email";
      const tenantReference =
        typeof request.body?.tenantId === "string" && request.body.tenantId.trim().length > 0
          ? request.body.tenantId.trim().toLowerCase()
          : "unknown-tenant";
