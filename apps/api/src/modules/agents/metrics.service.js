...
function extractToolCost(metadata) {
    if (!metadata || typeof metadata !== "object") {
        return 0;
    }
    const candidate = metadata.toolCost;
...
async function loadRunLogs(input) {
    ...
    return rows
        .filter((row) => row.status !== "RUNNING" && row.status !== "WAITING_APPROVAL")
        .map((row) => ({
        agentId: row.agentId,
...
export class AgentMetricsService {
    ...
    async getMetrics(input) {
        ...
        const logs = [
            ...
            ...this.runLogs.filter((log) => log.agentId === input.agentId &&
    ...
    async detectFailRateAlerts(tenantId, threshold = 0.2, windowMinutes = 5) {
        ...
        const tenantLogs = [
            ...
            ...this.runLogs.filter((log) => log.tenantId === tenantId && log.createdAt.getTime() >= since.getTime())
        ];
        const grouped = new Map();
        for (const log of tenantLogs) {
    ...
    async exportCsv(tenantId, agentId) {
        const rows = [
            ...(await loadRunLogs({
                ...(agentId ? { agentId } : {}),
                tenantId
