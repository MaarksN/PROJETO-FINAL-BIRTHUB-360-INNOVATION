...
function assertConversationsListBody(body) {
...
function assertConversationMutationBody(body) {
    assert.equal(typeof body, "object");
    assert.notEqual(body, null);
    const candidate = body;
...
void test("conversations router lists scoped threads with filters and previews", async () => {
    ...
    const restore = stubMethod(prisma.conversationThread, "findMany", (args) => {
...
void test("conversations router creates a thread and reloads the scoped detail", async () => {
    let createdThreadPayload = null;
    let createdMessagePayload = null;
    const tx = {
        conversationMessage: {
            create: (args) => {
    ...
    const restores = [
        ...
        stubMethod(prisma.conversationThread, "findFirst", () => Promise.resolve({
            channel: "internal",
...
void test("conversations router updates status inside the authenticated tenant scope", async () => {
    let receivedScopeLookup = null;
    let receivedUpdateArgs = null;
    const restores = [
        stubMethod(prisma.conversationThread, "findFirst", (args) => {
            receivedScopeLookup = args;
            return Promise.resolve({
                id: "thread_1"
            });
        }),
        stubMethod(prisma.conversationThread, "update", (args) => {
