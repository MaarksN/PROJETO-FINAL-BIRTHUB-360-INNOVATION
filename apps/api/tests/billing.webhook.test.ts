...
void test("stripe webhook accepts signatures signed with a fallback secret during rotation", async () => {
  const config = {
    ...createTestApiConfig(),
    STRIPE_WEBHOOK_SECRET_FALLBACKS: "whsec_previous_rotation",
    stripeWebhookSecretFallbacks: ["whsec_previous_rotation"]
  };
  const stripe = new Stripe(config.STRIPE_SECRET_KEY, {
    apiVersion: STRIPE_API_VERSION
  });
  ...
  const signature = stripe.webhooks.generateTestHeaderString({
    payload,
    secret: "whsec_previous_rotation"
  });
  const billingEvents = createBillingEventStubs();
