...
function createBillingEventStubs() {
    ...
    return {
        get: (eventId) => events.get(eventId) ?? null,
        restores: [
...
void test("stripe webhook accepts valid signature and records billing event state", async () => {
    ...
    try {
        ...
        assert.equal(billingEvents.get("evt_valid_signature")?.status, BillingEventStatus.processed);
        assert.equal(billingEvents.get("evt_valid_signature")?.attemptCount, 1);
    }
    finally {
