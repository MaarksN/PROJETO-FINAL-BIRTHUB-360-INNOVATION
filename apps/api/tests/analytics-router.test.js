...
function assertDateRangeInput(value) {
    assert.equal(typeof value, "object");
    assert.notEqual(value, null);
}
function createAnalyticsTestApp(role = Role.ADMIN) {
...
void test("analytics router parses usage date range filters before delegating to the service", async () => {
    let received = null;
    const restore = stubMethod(analyticsRouterService, "getUsageMetrics", (input) => {
        received = input;
...
void test("analytics router exports billing csv for super admins", async () => {
    let received = null;
    const restore = stubMethod(analyticsRouterService, "exportBillingCsv", (input) => {
        received = input;
