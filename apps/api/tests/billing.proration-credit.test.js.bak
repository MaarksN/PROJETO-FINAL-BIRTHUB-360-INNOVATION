...
void test("customer.subscription.updated creates one proration credit for a downgrade", async () => {
    ...
    const restores = [
        ...
        stubMethod(prisma.billingCredit, "create", (args) => {
    ...
    try {
        ...
        assert.equal(billingEvents.get("evt_subscription_downgrade_1")?.status, BillingEventStatus.processed);
    }
    finally {
