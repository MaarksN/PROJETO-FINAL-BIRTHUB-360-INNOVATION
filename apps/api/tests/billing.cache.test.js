...
import { configureCacheStore, deleteCacheKeys, readCacheValue, setCacheStoreForTests, writeCacheValue } from "../src/common/cache/cache-store.js";
...
void test("cache configuration rejects missing Redis in production", () => {
    assert.throws(() => configureCacheStore(undefined, "production"), /CACHE_CONFIGURATION_INVALID/);
...
void test("cache read, write and delete fail closed in production mode", async () => {
    ...
    try {
        ...
        await assert.rejects(() => writeCacheValue("billing:tenant_1", '{"status":"active"}', 60), /CACHE_WRITE_FAILED/);
        await assert.rejects(() => deleteCacheKeys(["billing:tenant_1"]), /CACHE_DELETE_FAILED/);
    }
    finally {
        configureCacheStore(undefined, "test");
        setCacheStoreForTests(null);
