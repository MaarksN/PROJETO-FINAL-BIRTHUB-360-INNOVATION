...
- `apps/web` vs `apps/legacy/dashboard`
...
2. `@birthub/db` permanece como schema legado de CRM e fica isolado em `apps/legacy/dashboard`, `apps/api-gateway`, `apps/agent-orchestrator` e `packages/db` até a migração dirigida.
3. `apps/web` é a interface canônica de produto e release. `apps/legacy/dashboard` é apenas quarentena documental e não entra na lane core.
...
2. Manter `apps/legacy/dashboard` apenas como marcador de quarentena ou absorver definitivamente qualquer fluxo remanescente em `apps/web`.
