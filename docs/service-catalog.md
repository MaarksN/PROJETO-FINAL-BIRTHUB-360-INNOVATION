...
- Apenas `apps/web`, `apps/api`, `apps/worker`, `packages/database` e `packages/agent-packs` são **core canônico**.
...
| Core | `packages/agent-packs` | Catálogo de agentes | 🟢 Dependência de domínio integrada ao fluxo transacional | AI Platform | `apps/api/src/modules/marketplace/marketplace-service.ts`; `apps/worker/src/agents/runtime.shared.ts` |
...
1. **Core canônico oficial:** `apps/web`, `apps/api`, `apps/worker`, `packages/database`, `packages/agent-packs`.
