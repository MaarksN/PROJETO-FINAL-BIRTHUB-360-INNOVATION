...
import { apiProductCapabilityEnvSchema, mapApiProductCapabilities } from "./product-capabilities.js";
import { commaSeparatedList, envBoolean, EnvValidationError, hasPlaceholderMarker, hasRequiredPostgresSsl, hasRequiredRedisTls, isLocalUrl, isStripeTestSecretKey, isSecureHttpUrl, nodeEnvSchema, nonEmptyString, optionalNonEmptyString, optionalUrlString, parseEnv, urlString } from "./shared.js";
...
function collectProductionSecretIssues(parsed) {
    const issues = [];
...
function collectProductionUrlIssues(parsed, corsOrigins) {
    const issues = [];
    ...
    if (corsOrigins.length === 0 ||
        corsOrigins.some((origin) => origin === "*" || origin.includes("localhost") || origin.includes("127.0.0.1"))) {
        issues.push("API_CORS_ORIGINS must only contain approved production origins.");
    }
    return issues;
}
function collectProductionInfrastructureIssues(parsed, deploymentEnvironment) {
    const issues = [];
...
function validateProductionApiConfig(parsed, corsOrigins, deploymentEnvironment) {
    const issues = [
...
export function getApiConfig(env = getEnvironmentSource()) {
    ...
    const deploymentEnvironment = runtimeEnvironment.DEPLOYMENT_ENVIRONMENT === "staging"
        ? "staging"
