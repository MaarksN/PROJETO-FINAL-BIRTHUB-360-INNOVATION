...
function parseContractOverridesFromObject(
  ...
  if (retryRecord) {
    ...
    if (typeof retryRecord.maxAttempts === "number") {
      retryOverride.maxAttempts = retryRecord.maxAttempts;
    }
  ...
  if (observability) {
    ...
    if (events || metrics) {
      overrides.observability = {
        events:
          events?.filter((entry): entry is BoardPrepContract["observability"]["events"][number] =>
        ...
        metrics:
          metrics?.filter((entry): entry is BoardPrepContract["observability"]["metrics"][number] =>
...
function parseContractOverrides(rawText: string): Partial<BoardPrepContract> {
  ...
  const eventValues = extractYamlArray(rawText, ["events"]);
