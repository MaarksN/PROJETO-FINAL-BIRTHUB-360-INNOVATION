...
const VALID_INPUT: PipelineOracleInput = {
  ...
  sections: ["forecast", "coverage", "deal_risk"],
  segments: ["enterprise", "mid_market", "commercial"],
  targetArr: 15_000_000,
...
void test("PipelineOracle returns success output on happy path", async () => {
  ...
  assert.ok(output.pipelineBrief.forecastSignals.length >= 2);
  assert.ok(output.observability.metrics.toolCalls >= 3);
  assert.ok(
    output.observability.events.some(
      (event: PipelineEvent) => event.name === "pipelineoracle.response.generated"
    )
  );
});
...
void test("PipelineOracle returns error when contract mode is hard_fail", async () => {
  ...
  const failingAdapters: PipelineOracleToolAdapters = {
    async fetchDealRisk() {
      throw new Error("deal risk unavailable");
    },
    async fetchPipelineHealth() {
      throw new Error("pipeline health unavailable");
    },
    async fetchRevenueAttainment() {
      throw new Error("revenue attainment unavailable");
    }
  };
  ...
  try {
    ...
    assert.ok(
      output.observability.events.some(
        (event: PipelineEvent) => event.name === "pipelineoracle.fallback.applied"
      )
    );
  } finally {
