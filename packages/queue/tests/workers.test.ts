...
void test("CampaignWorker succeeds when payload is valid", async () => {
...
void test("ReportWorker supports async heavy report execution", async () => {
  const worker = new ReportWorker(3);
  const result = await worker.run({ reportId: "r1", type: "board" });
  assert.equal(result.status, "ok");
});

void test("WebhookWorker uses DLQ for persistent delivery failure", async () => {
