...
import { agentManifestSchema, MANIFEST_VERSION } from './schema.ts';
...
export class AgentManifestParseError extends Error {
  ...
  constructor(issues: string[]) {
    super(`Agent manifest inválido: ${issues.join('; ')}`);
...
export function parseAgentManifest(input: unknown): AgentManifest {
  ...
  if (!result.success) {
    ...
    if (typeof version === 'string' && version !== MANIFEST_VERSION) {
      issues.unshift(
        `manifestVersion: versão incompatível (${version}). Esperado ${MANIFEST_VERSION}.`,
      );
    }
