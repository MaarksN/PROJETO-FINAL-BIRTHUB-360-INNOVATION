...
function toPremiumLayerStatus(score) {
    if (score >= 85) {
        return "elite";
    }
    if (score >= 70) {
...
function containsOpportunitySignal(signals) {
    return signals.some((signal) => /(upsell|cross[- ]sell|expansion|pipeline|lead|roi|growth|efficien|win|upgrade|adoption|campaign)/i.test(signal));
}
function containsRiskSignal(signals) {
    return signals.some((signal) => /(risk|churn|incident|critical|breach|drop|delay|blocked|escalat|fraud|complaint|ticket|outage)/i.test(signal));
}
function getSegmentConfidenceScore(confidence) {
    if (confidence === "high") {
        return 94;
    }
...
function createPremiumLayerContext(input) {
    ...
    return {
        ...
        opportunitySignalsDetected: containsOpportunitySignal(textSignals) || input.numericSummary.trend === "up",
...
function buildSignalFusionLayer(input, context) {
    return {
        id: "signal-fusion",
        name: "Signal Fusion",
...
function buildSegmentModelingLayer(input, context) {
    return {
        id: "segment-modeling",
...
function buildMemoryGridLayer(context) {
    return {
        id: "memory-grid",
        name: "Memory Grid",
...
function buildOpportunityRadarLayer(input, context) {
    return {
...
function buildCollaborationGraphLayer(context) {
    return {
        ...
        score: clampScore(55 + Math.min(context.collaborationTargets.length * 10, 30) + (context.workflowReady ? 10 : 0)),
...
function buildGovernanceShieldLayer(context) {
    return {
...
function buildWorkflowAutomationLayer(input, context) {
    return {
...
function buildAdaptiveLearningLayer(context) {
    return {
...
export function buildPremiumLayersAssessment(input) {
...
export function summarizePremiumLayers(layers) {
    if (layers.length === 0) {
        return {
            needsAttention: [],
            overallScore: 0,
    ...
    const overallScore = layers.reduce((total, layer) => total + layer.score, 0) / Math.max(layers.length, 1);
    return {
...
export function describeCapabilityIntent(capabilityType) {
...
export function inferCollaborationTargets(manifest) {
    const targets = [
        ...manifest.keywords.filter((keyword) => /(planner|implementer|reviewer|analyst|manager|architect|engineer|concierge)/i.test(keyword)),
...
export function buildRecommendedActions(input) {
    ...
    if (input.numericSummary.count > 0) {
        actions.push({
            ...
            reason: input.numericSummary.outlierCount > 0
...
export function buildMemoryKey(agentId, segmentProfile, suffix) {
