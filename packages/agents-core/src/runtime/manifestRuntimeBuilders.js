import { buildEmergingRisks, buildLeadingIndicators, buildOutputConfidence, buildPreventiveActionPlan, buildStatus, deriveSpecialistDeliverables, hasWorkflowContext, inferOutputGovernance, isRecord, normalizeJsonValue, readObjective, readPrimaryOwner, readTriggerSource, summarizeLearning, uniqueStrings } from "./manifestRuntimeCore.js";
import { buildMemoryKey, buildPremiumLayersAssessment, buildRecommendedActions, inferCollaborationTargets, inferSegmentProfile, readNumericSignals, readTextSignals, summarizeNumericSignals, summarizePremiumLayers } from "./intelligence.js";
function collectFocusDomains(output, focusDomains) {
...
function collectRecommendedAgents(output, recommendedAgents) {
...
function collectWorkflowSteps(output, workflowSteps) {
    ...
    for (const value of output.workflowPlan) {
        ...
        workflowSteps.push({
            agent_id: value.agentId,
            expected_outcome: typeof value.expectedOutcome === "string"
...
function readMeshBlueprint(steps) {
    const focusDomains = [];
...
function buildSuggestedHandoffs(input) {
    const manifestSupportsHandoff = input.manifest.tools.some((tool) => /(agent|handoff|delegate)/i.test(`${tool.id} ${tool.name}`));
    if (!manifestSupportsHandoff && input.collaborationTargets.length === 0) {
        return [];
    }
    const targets = input.collaborationTargets.length > 0
...
export function buildAgentRuntimeOutput(input) {
    ...
    return {
        ...
        decisions_to_anticipate: buildDecisionsToAnticipate({
            owner,
            recommendedAction: recommendedActions[0]?.action ?? "Executar o proximo passo priorizado pelo agente.",
            suggestedHandoffs
        }),
