...
import {
  ...
  type BottleneckDetectorContract,
  BottleneckDetectorContractSchema,
  type BottleneckDetectorInput,
  BottleneckDetectorInputSchema,
  type BottleneckDetectorOutput,
  BottleneckDetectorOutputSchema,
  type QuotaEvent,
...
import {
  ...
  createDefaultBottleneckDetectorToolAdapters,
  normalizeQuotaToolId,
  QUOTAARCHITECT_TOOL_IDS,
  type BottleneckDetectorToolAdapters,
  type QuotaToolId,
  type QuotaToolInput,
...
interface BottleneckDetectorAgentOptions {
  contractPath?: string;
  now?: () => Date;
  sleep?: (delayMs: number) => Promise<void>;
  toolAdapters?: BottleneckDetectorToolAdapters;
}

interface LoadedContract {
  contract: BottleneckDetectorContract;
...
function classifyContractSource(contractPath: string): LoadedContract["source"] {
  ...
  if (
    normalized.endsWith(
      path
        .join("audit", "pending_review", "ciclo1_bottleneckdetector", "contract.yaml")
        .toLowerCase()
    )
  ) {
    return "audit_file";
  ...
  if (
    normalized.endsWith(
      path.join("executives", "BottleneckDetector", "contract.yaml").toLowerCase()
    )
  ) {
    return "file";
  }
...
function parseContractOverridesFromObject(
...
): Partial<BottleneckDetectorContract> {
  const overrides: Partial<BottleneckDetectorContract> = {};
  ...
  if (retry) {
    const retryOverride: Partial<BottleneckDetectorContract["retry"]> = {};
    ...
    if (Object.keys(retryOverride).length > 0) {
      overrides.retry = retryOverride as BottleneckDetectorContract["retry"];
    }
  }
  ...
  if (observability) {
    ...
    if (events || metrics) {
      const filteredEvents = events?.filter(
        (entry): entry is BottleneckDetectorContract["observability"]["events"][number] =>
          DEFAULT_QUOTAARCHITECT_CONTRACT.observability.events.includes(
            entry as BottleneckDetectorContract["observability"]["events"][number]
          )
      );
      const filteredMetrics = metrics?.filter(
        (entry): entry is BottleneckDetectorContract["observability"]["metrics"][number] =>
          DEFAULT_QUOTAARCHITECT_CONTRACT.observability.metrics.includes(
            entry as BottleneckDetectorContract["observability"]["metrics"][number]
          )
      );
...
function parseContractOverrides(rawText: string): Partial<BottleneckDetectorContract> {
...
function mergeContract(
  overrides: Partial<BottleneckDetectorContract>,
  fallback: BottleneckDetectorContract
): BottleneckDetectorContract {
  return BottleneckDetectorContractSchema.parse({
    ...fallback,
    ...overrides,
    observability: {
...
export class BottleneckDetectorAgent {
  ...
  private readonly toolAdapters: BottleneckDetectorToolAdapters;
  ...
  constructor(options: BottleneckDetectorAgentOptions = {}) {
    this.contractPaths = options.contractPath
      ? [options.contractPath]
      : [...DEFAULT_CONTRACT_PATHS];
    this.now = options.now ?? (() => new Date());
    this.sleepFn = options.sleep ?? sleep;
    this.toolAdapters = options.toolAdapters ?? createDefaultBottleneckDetectorToolAdapters();
  }
  ...
  async run(input: BottleneckDetectorInput): Promise<BottleneckDetectorOutput> {
    const parsedInput = BottleneckDetectorInputSchema.parse(input);
    ...
    emitEvent({
      details: {
        source: "request",
        toolId: "bottleneckdetector"
      },
      level: "info",
      message: "BottleneckDetector request accepted.",
      name: "bottleneckdetector.request.received"
    });
    ...
    emitEvent({
      ...
      name: "bottleneckdetector.contract.loaded"
    });
    ...
    const runWithRetry = async <T>(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        emitEvent({
          ...
          name: "bottleneckdetector.tool.call.started"
        });
        ...
        try {
          ...
          emitEvent({
            ...
            name: "bottleneckdetector.tool.call.succeeded"
          });
          return result;
        } catch (error) {
          const asError =
          ...
          emitEvent({
            ...
            name: "bottleneckdetector.tool.call.failed"
          });
          ...
          emitEvent({
            ...
            name: "bottleneckdetector.retry.scheduled"
          });
    ...
    if (fallbackApplied) {
      emitEvent({
        ...
        level: "warning",
        message: "Fallback behavior applied for BottleneckDetector output.",
        name: "bottleneckdetector.fallback.applied"
      });
    }
    ...
    const status: BottleneckDetectorOutput["status"] =
    ...
    emitEvent({
      ...
      message: "BottleneckDetector response generated.",
      name: "bottleneckdetector.response.generated"
    });

    const output = BottleneckDetectorOutputSchema.parse({
      ...
      summary: fallbackApplied
        ? "BottleneckDetector generated under fallback mode due to tool failures."
        : "BottleneckDetector generated with complete quota planning coverage."
    });
