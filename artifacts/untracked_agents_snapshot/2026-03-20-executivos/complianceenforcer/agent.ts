...
import {
  ...
  type ComplianceEnforcerContract,
  ComplianceEnforcerContractSchema,
  type ComplianceEnforcerInput,
  ComplianceEnforcerInputSchema,
  type ComplianceEnforcerOutput,
  ComplianceEnforcerOutputSchema,
  DEFAULT_COMPETITORXRAY_CONTRACT
} from "./schemas.js";
import {
  COMPETITORXRAY_TOOL_IDS,
  CompetitorIntelSnapshotSchema,
  createDefaultComplianceEnforcerToolAdapters,
  ...
  type ComplianceEnforcerToolAdapters
} from "./tools.js";
...
interface ComplianceEnforcerAgentOptions {
  contractPath?: string;
  now?: () => Date;
  sleep?: (delayMs: number) => Promise<void>;
  toolAdapters?: ComplianceEnforcerToolAdapters;
}

interface LoadedContract {
  contract: ComplianceEnforcerContract;
...
function classifyContractSource(contractPath: string): LoadedContract["source"] {
  ...
  if (
    normalized.endsWith(
      path
        .join("audit", "pending_review", "ciclo1_complianceenforcer", "contract.yaml")
        .toLowerCase()
    )
  ) {
    return "audit_file";
  ...
  if (
    normalized.endsWith(path.join("executives", "ComplianceEnforcer", "contract.yaml").toLowerCase())
  ) {
    return "file";
  }
  return "custom_file";
...
function parseContractOverridesFromObject(
...
): Partial<ComplianceEnforcerContract> {
  const overrides: Partial<ComplianceEnforcerContract> = {};
  ...
  if (retry) {
    const retryOverride: Partial<ComplianceEnforcerContract["retry"]> = {};
    ...
    if (Object.keys(retryOverride).length > 0) {
      overrides.retry = retryOverride as ComplianceEnforcerContract["retry"];
    }
  }
  ...
  if (observability) {
    ...
    if (events || metrics) {
      const filteredEvents = events?.filter(
        (entry): entry is ComplianceEnforcerContract["observability"]["events"][number] =>
          DEFAULT_COMPETITORXRAY_CONTRACT.observability.events.includes(
            entry as ComplianceEnforcerContract["observability"]["events"][number]
          )
      );
      const filteredMetrics = metrics?.filter(
        (entry): entry is ComplianceEnforcerContract["observability"]["metrics"][number] =>
          DEFAULT_COMPETITORXRAY_CONTRACT.observability.metrics.includes(
            entry as ComplianceEnforcerContract["observability"]["metrics"][number]
          )
      );
...
function parseContractOverrides(rawText: string): Partial<ComplianceEnforcerContract> {
...
function mergeContract(
  overrides: Partial<ComplianceEnforcerContract>,
  fallback: ComplianceEnforcerContract
): ComplianceEnforcerContract {
  return ComplianceEnforcerContractSchema.parse({
    ...fallback,
    ...overrides,
    observability: {
...
export class ComplianceEnforcerAgent {
  ...
  private readonly toolAdapters: ComplianceEnforcerToolAdapters;
  ...
  constructor(options: ComplianceEnforcerAgentOptions = {}) {
    this.contractPaths = options.contractPath
      ? [options.contractPath]
      : [...DEFAULT_CONTRACT_PATHS];
    this.now = options.now ?? (() => new Date());
    this.sleepFn = options.sleep ?? sleep;
    this.toolAdapters = options.toolAdapters ?? createDefaultComplianceEnforcerToolAdapters();
  }
  ...
  async run(input: ComplianceEnforcerInput): Promise<ComplianceEnforcerOutput> {
    const parsedInput = ComplianceEnforcerInputSchema.parse(input);
    ...
    emitEvent({
      details: {
        source: "request",
        toolId: "complianceenforcer"
      },
      level: "info",
      message: "ComplianceEnforcer request accepted.",
      name: "complianceenforcer.request.received"
    });
    ...
    emitEvent({
      ...
      name: "complianceenforcer.contract.loaded"
    });
    ...
    const runWithRetry = async <T>(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        emitEvent({
          ...
          name: "complianceenforcer.tool.call.started"
        });
        ...
        try {
          ...
          emitEvent({
            ...
            name: "complianceenforcer.tool.call.succeeded"
          });
          return result;
        } catch (error) {
          const asError =
          ...
          emitEvent({
            ...
            name: "complianceenforcer.tool.call.failed"
          });
          ...
          emitEvent({
            ...
            name: "complianceenforcer.retry.scheduled"
          });
    ...
    if (fallbackApplied) {
      emitEvent({
        ...
        level: "warning",
        message: "Fallback behavior applied for ComplianceEnforcer output.",
        name: "complianceenforcer.fallback.applied"
      });
    }
    ...
    const status: ComplianceEnforcerOutput["status"] =
    ...
    emitEvent({
      ...
      message: "ComplianceEnforcer response generated.",
      name: "complianceenforcer.response.generated"
    });

    const output = ComplianceEnforcerOutputSchema.parse({
      ...
      summary: fallbackApplied
        ? "ComplianceEnforcer generated under fallback mode due to tool failures."
        : "ComplianceEnforcer generated with complete competitor, pricing, and feature-gap signal coverage."
    });
