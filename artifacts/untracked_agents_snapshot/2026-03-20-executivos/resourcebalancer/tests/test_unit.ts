...
import type { ResourceBalancerToolAdapters } from "../tools.js";

const VALID_INPUT: ResourceBalancerInput = {
  constraints: {
    currency: "BRL",
    language: "pt-BR",
    maxActions: 4
  },
  requestId: "req-resourcebalancer-unit-001",
...
void test("ResourceBalancer returns success output on happy path", async () => {
  ...
  assert.ok(
    output.observability.events.some(
      (event: BudgetEvent) => event.name === "resourcebalancer.response.generated"
    )
  );
});

void test("ResourceBalancer returns error when contract mode is hard_fail", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "resourcebalancer-contract-"));
  ...
  const failingAdapters: ResourceBalancerToolAdapters = {
  ...
  try {
    ...
    assert.ok(
      output.observability.events.some(
        (event: BudgetEvent) => event.name === "resourcebalancer.fallback.applied"
      )
    );
  } finally {
