...
import {
  ...
  type ResourceBalancerContract,
  ResourceBalancerContractSchema,
  type ResourceBalancerInput,
  ResourceBalancerInputSchema,
  type ResourceBalancerOutput,
  ResourceBalancerOutputSchema,
  type BudgetMetrics,
  DEFAULT_BUDGETFLUID_CONTRACT
} from "./schemas.js";
import {
  BUDGETFLUID_TOOL_IDS,
  type ResourceBalancerToolAdapters,
  type BudgetToolId,
  type BudgetToolInput,
  createDefaultResourceBalancerToolAdapters,
...
interface ResourceBalancerAgentOptions {
  contractPath?: string;
  now?: () => Date;
  sleep?: (delayMs: number) => Promise<void>;
  toolAdapters?: ResourceBalancerToolAdapters;
}

interface LoadedContract {
  contract: ResourceBalancerContract;
...
function classifyContractSource(contractPath: string): LoadedContract["source"] {
  ...
  if (
    normalized.endsWith(
      path
        .join("audit", "pending_review", "ciclo1_resourcebalancer", "contract.yaml")
        .toLowerCase()
    )
  ) {
    return "audit_file";
  ...
  if (
    normalized.endsWith(path.join("executives", "ResourceBalancer", "contract.yaml").toLowerCase())
  ) {
    return "file";
  }
  return "custom_file";
...
function parseContractOverridesFromObject(
...
): Partial<ResourceBalancerContract> {
  const overrides: Partial<ResourceBalancerContract> = {};
  ...
  if (retry) {
    const retryOverride: Partial<ResourceBalancerContract["retry"]> = {};
    ...
    if (Object.keys(retryOverride).length > 0) {
      overrides.retry = retryOverride as ResourceBalancerContract["retry"];
    }
  }
  ...
  if (observability) {
    ...
    if (events || metrics) {
      const filteredEvents = events?.filter(
        (entry): entry is ResourceBalancerContract["observability"]["events"][number] =>
          DEFAULT_BUDGETFLUID_CONTRACT.observability.events.includes(
            entry as ResourceBalancerContract["observability"]["events"][number]
          )
      );
      const filteredMetrics = metrics?.filter(
        (entry): entry is ResourceBalancerContract["observability"]["metrics"][number] =>
          DEFAULT_BUDGETFLUID_CONTRACT.observability.metrics.includes(
            entry as ResourceBalancerContract["observability"]["metrics"][number]
          )
      );
...
function parseContractOverrides(rawText: string): Partial<ResourceBalancerContract> {
...
function mergeContract(
  overrides: Partial<ResourceBalancerContract>,
  fallback: ResourceBalancerContract
): ResourceBalancerContract {
  return ResourceBalancerContractSchema.parse({
    ...fallback,
    ...overrides,
    observability: {
...
export class ResourceBalancerAgent {
  ...
  private readonly toolAdapters: ResourceBalancerToolAdapters;
  ...
  constructor(options: ResourceBalancerAgentOptions = {}) {
    this.contractPaths = options.contractPath
      ? [options.contractPath]
      : [...DEFAULT_CONTRACT_PATHS];
    this.now = options.now ?? (() => new Date());
    this.sleepFn = options.sleep ?? sleep;
    this.toolAdapters = options.toolAdapters ?? createDefaultResourceBalancerToolAdapters();
  }
  ...
  async run(input: ResourceBalancerInput): Promise<ResourceBalancerOutput> {
    const parsedInput = ResourceBalancerInputSchema.parse(input);
    ...
    emitEvent({
      details: {
        source: "request",
        toolId: "resourcebalancer"
      },
      level: "info",
      message: "ResourceBalancer request accepted.",
      name: "resourcebalancer.request.received"
    });
    ...
    emitEvent({
      ...
      name: "resourcebalancer.contract.loaded"
    });
    ...
    const runWithRetry = async <T>(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        emitEvent({
          ...
          name: "resourcebalancer.tool.call.started"
        });
        ...
        try {
          ...
          emitEvent({
            ...
            name: "resourcebalancer.tool.call.succeeded"
          });
          return result;
        } catch (error) {
          const asError =
          ...
          emitEvent({
            ...
            name: "resourcebalancer.tool.call.failed"
          });
          ...
          emitEvent({
            ...
            name: "resourcebalancer.retry.scheduled"
          });
    ...
    if (fallbackApplied) {
      emitEvent({
        ...
        level: "warning",
        message: "Fallback behavior applied for ResourceBalancer output.",
        name: "resourcebalancer.fallback.applied"
      });
    }
    ...
    const status: ResourceBalancerOutput["status"] =
    ...
    emitEvent({
      ...
      message: "ResourceBalancer response generated.",
      name: "resourcebalancer.response.generated"
    });

    const output = ResourceBalancerOutputSchema.parse({
      agent: "ResourceBalancer",
      budgetBrief: {
        actions: [
          {
            owner: "CFO",
      ...
      summary: fallbackApplied
        ? "ResourceBalancer generated under fallback mode due to tool failures."
        : "ResourceBalancer generated with complete budget telemetry and scenario coverage."
    });
