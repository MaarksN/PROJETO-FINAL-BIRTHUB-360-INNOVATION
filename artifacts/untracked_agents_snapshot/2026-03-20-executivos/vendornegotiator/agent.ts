...
import {
  ...
  type VendorNegotiatorContract,
  VendorNegotiatorContractSchema,
  type VendorNegotiatorInput,
  VendorNegotiatorInputSchema,
  type VendorNegotiatorOutput,
  VendorNegotiatorOutputSchema,
  type BudgetMetrics,
  DEFAULT_BUDGETFLUID_CONTRACT
} from "./schemas.js";
import {
  BUDGETFLUID_TOOL_IDS,
  type VendorNegotiatorToolAdapters,
  type BudgetToolId,
  type BudgetToolInput,
  createDefaultVendorNegotiatorToolAdapters,
...
interface VendorNegotiatorAgentOptions {
  contractPath?: string;
  now?: () => Date;
  sleep?: (delayMs: number) => Promise<void>;
  toolAdapters?: VendorNegotiatorToolAdapters;
}

interface LoadedContract {
  contract: VendorNegotiatorContract;
...
function classifyContractSource(contractPath: string): LoadedContract["source"] {
  ...
  if (
    normalized.endsWith(
      path
        .join("audit", "pending_review", "ciclo1_vendornegotiator", "contract.yaml")
        .toLowerCase()
    )
  ) {
    return "audit_file";
  ...
  if (
    normalized.endsWith(path.join("executives", "VendorNegotiator", "contract.yaml").toLowerCase())
  ) {
    return "file";
  }
  return "custom_file";
...
function parseContractOverridesFromObject(
...
): Partial<VendorNegotiatorContract> {
  const overrides: Partial<VendorNegotiatorContract> = {};
  ...
  if (retry) {
    const retryOverride: Partial<VendorNegotiatorContract["retry"]> = {};
    ...
    if (Object.keys(retryOverride).length > 0) {
      overrides.retry = retryOverride as VendorNegotiatorContract["retry"];
    }
  }
  ...
  if (observability) {
    ...
    if (events || metrics) {
      const filteredEvents = events?.filter(
        (entry): entry is VendorNegotiatorContract["observability"]["events"][number] =>
          DEFAULT_BUDGETFLUID_CONTRACT.observability.events.includes(
            entry as VendorNegotiatorContract["observability"]["events"][number]
          )
      );
      const filteredMetrics = metrics?.filter(
        (entry): entry is VendorNegotiatorContract["observability"]["metrics"][number] =>
          DEFAULT_BUDGETFLUID_CONTRACT.observability.metrics.includes(
            entry as VendorNegotiatorContract["observability"]["metrics"][number]
          )
      );
...
function parseContractOverrides(rawText: string): Partial<VendorNegotiatorContract> {
...
function mergeContract(
  overrides: Partial<VendorNegotiatorContract>,
  fallback: VendorNegotiatorContract
): VendorNegotiatorContract {
  return VendorNegotiatorContractSchema.parse({
    ...fallback,
    ...overrides,
    observability: {
...
export class VendorNegotiatorAgent {
  ...
  private readonly toolAdapters: VendorNegotiatorToolAdapters;
  ...
  constructor(options: VendorNegotiatorAgentOptions = {}) {
    this.contractPaths = options.contractPath
      ? [options.contractPath]
      : [...DEFAULT_CONTRACT_PATHS];
    this.now = options.now ?? (() => new Date());
    this.sleepFn = options.sleep ?? sleep;
    this.toolAdapters = options.toolAdapters ?? createDefaultVendorNegotiatorToolAdapters();
  }
  ...
  async run(input: VendorNegotiatorInput): Promise<VendorNegotiatorOutput> {
    const parsedInput = VendorNegotiatorInputSchema.parse(input);
    ...
    emitEvent({
      details: {
        source: "request",
        toolId: "vendornegotiator"
      },
      level: "info",
      message: "VendorNegotiator request accepted.",
      name: "vendornegotiator.request.received"
    });
    ...
    emitEvent({
      ...
      name: "vendornegotiator.contract.loaded"
    });
    ...
    const runWithRetry = async <T>(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        emitEvent({
          ...
          name: "vendornegotiator.tool.call.started"
        });
        ...
        try {
          ...
          emitEvent({
            ...
            name: "vendornegotiator.tool.call.succeeded"
          });
          return result;
        } catch (error) {
          const asError =
          ...
          emitEvent({
            ...
            name: "vendornegotiator.tool.call.failed"
          });
          ...
          emitEvent({
            ...
            name: "vendornegotiator.retry.scheduled"
          });
    ...
    if (fallbackApplied) {
      emitEvent({
        ...
        level: "warning",
        message: "Fallback behavior applied for VendorNegotiator output.",
        name: "vendornegotiator.fallback.applied"
      });
    }
    ...
    const status: VendorNegotiatorOutput["status"] =
    ...
    emitEvent({
      ...
      message: "VendorNegotiator response generated.",
      name: "vendornegotiator.response.generated"
    });

    const output = VendorNegotiatorOutputSchema.parse({
      agent: "VendorNegotiator",
      budgetBrief: {
        actions: [
          {
            owner: "CFO",
      ...
      summary: fallbackApplied
        ? "VendorNegotiator generated under fallback mode due to tool failures."
        : "VendorNegotiator generated with complete budget telemetry and scenario coverage."
    });
