...
import type { VendorNegotiatorToolAdapters } from "../tools.js";

const VALID_INPUT: VendorNegotiatorInput = {
  constraints: {
    currency: "BRL",
    language: "pt-BR",
    maxActions: 4
  },
  requestId: "req-vendornegotiator-unit-001",
...
void test("VendorNegotiator returns success output on happy path", async () => {
  ...
  assert.ok(
    output.observability.events.some(
      (event: BudgetEvent) => event.name === "vendornegotiator.response.generated"
    )
  );
});

void test("VendorNegotiator returns error when contract mode is hard_fail", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "vendornegotiator-contract-"));
  ...
  const failingAdapters: VendorNegotiatorToolAdapters = {
  ...
  try {
    ...
    assert.ok(
      output.observability.events.some(
        (event: BudgetEvent) => event.name === "vendornegotiator.fallback.applied"
      )
    );
  } finally {
