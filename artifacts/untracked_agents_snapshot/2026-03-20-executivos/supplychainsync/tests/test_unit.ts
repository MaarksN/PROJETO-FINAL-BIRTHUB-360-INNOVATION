...
import type { SupplyChainSyncToolAdapters } from "../tools.js";

const VALID_INPUT: SupplyChainSyncInput = {
  constraints: {
    currency: "BRL",
    language: "pt-BR",
    maxActions: 4
  },
  requestId: "req-supplychainsync-unit-001",
...
void test("SupplyChainSync returns success output on happy path", async () => {
  ...
  assert.ok(
    output.observability.events.some(
      (event: MarketEvent) => event.name === "supplychainsync.response.generated"
    )
  );
});

void test("SupplyChainSync returns error when contract mode is hard_fail", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "supplychainsync-contract-"));
  ...
  const failingAdapters: SupplyChainSyncToolAdapters = {
  ...
  try {
    ...
    assert.ok(
      output.observability.events.some(
        (event: MarketEvent) => event.name === "supplychainsync.fallback.applied"
      )
    );
  } finally {
