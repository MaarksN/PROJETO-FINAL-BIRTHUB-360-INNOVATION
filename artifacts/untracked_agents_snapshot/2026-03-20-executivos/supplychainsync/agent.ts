...
import {
  ...
  type SupplyChainSyncContract,
  SupplyChainSyncContractSchema,
  type SupplyChainSyncInput,
  SupplyChainSyncInputSchema,
  type SupplyChainSyncOutput,
  SupplyChainSyncOutputSchema,
  type MarketEvent,
...
interface SupplyChainSyncAgentOptions {
  contractPath?: string;
  now?: () => Date;
  sleep?: (delayMs: number) => Promise<void>;
  toolAdapters?: SupplyChainSyncToolAdapters;
}

interface LoadedContract {
  contract: SupplyChainSyncContract;
...
function classifyContractSource(contractPath: string): LoadedContract["source"] {
  ...
  if (
    normalized.endsWith(
      path
        .join("audit", "pending_review", "ciclo1_supplychainsync", "contract.yaml")
        .toLowerCase()
    )
  ) {
    return "audit_file";
  ...
  if (
    normalized.endsWith(
      path.join("executives", "SupplyChainSync", "contract.yaml").toLowerCase()
    )
  ) {
    return "file";
  }
...
function parseContractOverridesFromObject(
...
): Partial<SupplyChainSyncContract> {
  const overrides: Partial<SupplyChainSyncContract> = {};
  ...
  if (retry) {
    const retryOverride: Partial<SupplyChainSyncContract["retry"]> = {};
    ...
    if (Object.keys(retryOverride).length > 0) {
      overrides.retry = retryOverride as SupplyChainSyncContract["retry"];
    }
  }
  ...
  if (observability) {
    ...
    if (events || metrics) {
      const filteredEvents = events?.filter(
        (entry): entry is SupplyChainSyncContract["observability"]["events"][number] =>
          DEFAULT_TRENDCATCHER_CONTRACT.observability.events.includes(
            entry as SupplyChainSyncContract["observability"]["events"][number]
          )
      );
      const filteredMetrics = metrics?.filter(
        (entry): entry is SupplyChainSyncContract["observability"]["metrics"][number] =>
          DEFAULT_TRENDCATCHER_CONTRACT.observability.metrics.includes(
            entry as SupplyChainSyncContract["observability"]["metrics"][number]
          )
      );
...
function parseContractOverrides(rawText: string): Partial<SupplyChainSyncContract> {
...
function mergeContract(
  overrides: Partial<SupplyChainSyncContract>,
  fallback: SupplyChainSyncContract
): SupplyChainSyncContract {
  return SupplyChainSyncContractSchema.parse({
    ...fallback,
    ...overrides,
    observability: {
...
export class SupplyChainSyncAgent {
  ...
  private readonly toolAdapters: SupplyChainSyncToolAdapters;
  ...
  constructor(options: SupplyChainSyncAgentOptions = {}) {
    this.contractPaths = options.contractPath
      ? [options.contractPath]
      : [...DEFAULT_CONTRACT_PATHS];
    this.now = options.now ?? (() => new Date());
    this.sleepFn = options.sleep ?? sleep;
    this.toolAdapters = options.toolAdapters ?? createDefaultSupplyChainSyncToolAdapters();
  }
  ...
  async run(input: SupplyChainSyncInput): Promise<SupplyChainSyncOutput> {
    const parsedInput = SupplyChainSyncInputSchema.parse(input);
    ...
    emitEvent({
      details: {
        source: "request",
        toolId: "supplychainsync"
      },
      level: "info",
      message: "SupplyChainSync request accepted.",
      name: "supplychainsync.request.received"
    });
    ...
    emitEvent({
      ...
      name: "supplychainsync.contract.loaded"
    });
    ...
    const runWithRetry = async <T>(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        emitEvent({
          ...
          name: "supplychainsync.tool.call.started"
        });
        ...
        try {
          ...
          emitEvent({
            ...
            name: "supplychainsync.tool.call.succeeded"
          });
          return result;
        } catch (error) {
          const asError =
          ...
          emitEvent({
            ...
            name: "supplychainsync.tool.call.failed"
          });
          ...
          emitEvent({
            ...
            name: "supplychainsync.retry.scheduled"
          });
    ...
    if (fallbackApplied) {
      emitEvent({
        ...
        level: "warning",
        message: "Fallback behavior applied for SupplyChainSync output.",
        name: "supplychainsync.fallback.applied"
      });
    }
    ...
    const status: SupplyChainSyncOutput["status"] =
    ...
    emitEvent({
      ...
      message: "SupplyChainSync response generated.",
      name: "supplychainsync.response.generated"
    });

    const output = SupplyChainSyncOutputSchema.parse({
      agent: "SupplyChainSync",
      domain: "executivos",
      fallback: {
      ...
      summary: fallbackApplied
        ? "SupplyChainSync generated under fallback mode due to tool failures."
        : "SupplyChainSync generated with complete trend and momentum signal coverage.",
      marketBrief: {
        actions: [
          {
            owner: "CMO",
